...
<body>
	...
	<p>★Session(브라우저 내에서는 계속해서 유지되는 정보, 주로 로그인 정보나 상태 정보 등으로 건 수가 많은 데이터가 아닌 경우)</p>
	<p>ApplicationContext(어플리케이션이 완전히 종료되지 않는 한 계속해서 유지됨 -> 버전 정보, 특정한 업데이트를 위한 값 외에는 저장 x)</p>
	
	<%
		pageContext.setAttribute("pageCtx", "JSW_PAGE_CTX");//페이지 내에서만 사용하는 객체
		request.setAttribute("req", "JSW_REQ");
		//Request -> 1. getParameter(URL 또는 FORM태그를 이용한 경우 내부 태그를 name으로 구분하여 파라메터가 String으로 들어있음)
		//    		 2. Attribute <- 어떤 요청을 통해서 오거나 Request가 Forward라는 기능으로 페이지 전환을 하는 경우 Object 들어있음
		session.setAttribute("session", "JSW_SESSION");
		application.setAttribute("app", "JSW_APPV1.1");
	%>
